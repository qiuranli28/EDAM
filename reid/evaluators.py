...
def check_and_replace_inf(data):
    if np.isinf(data).any():
        print("Data contains infinity values. Replacing with large finite value.")
        data[np.isinf(data)] = np.finfo(np.float32).max
    if np.isnan(data).any():
        print("Data contains NaN values. Replacing with zero.")
...
def pairwise_distance(matcher, prob_fea, gal_fea, gal_batch_size=128, prob_batch_size=128, verbose=False):
    with torch.inference_mode():
        ...
        for i in range(0, num_probs, prob_batch_size):
            ...
            if verbose:
                print('Query batch %d / %d ... \t' % (i // prob_batch_size + 1, num_probs // prob_batch_size + 1),
                      end='\r', file=sys.stdout.console)
        ...
        dist = -score
        dist = dist.cpu().float()
        dist = check_and_replace_inf(dist.numpy())  # 添加检查和替换
        dist = torch.tensor(dist, device='cuda:0')  # 转换为 PyTorch 张量
    return dist  # [p, g]
...
def evaluate_all(distmat, query=None, gallery=None,
    ...
    distmat = check_and_replace_inf(distmat.cpu().numpy())  # 添加检查和替换
    # Compute mean AP
...
def reranking(dist, query_num, k1=20, k2=6, lamda_value=0.3, verbose=False):
    ...
    del V
    del jaccard_dist
    final_dist = torch.tensor(final_dist[:query_num, query_num:], device='cuda:0')  # 转换为 PyTorch 张量
    return final_dist
...
class Evaluator(object):
    ...
    def evaluate(self, testset, query_loader, gallery_loader):
        ...
        if testset.has_time_info:
            dist_rerank = torch.zeros((num_all, num_all), device=dist.device)
            ...
            with torch.no_grad():
                ...
                dist_rerank[:num_prob, :num_prob] = pairwise_distance(self.matcher, prob_fea, prob_fea,
                                                                      self.test_gal_batch,
                                                                      self.test_prob_batch)
                dist_rerank[num_prob:, num_prob:] = pairwise_distance(self.matcher, gal_fea, gal_fea,
                                                                      self.test_gal_batch,
                                                                      self.test_prob_batch)
            ...
            dist_rerank = torch.zeros((num_all, num_all),
                                      device=dist.device)  # Create tensor with the same device as dist
            dist_tlift = torch.zeros((num_all, num_all),
                                     device=dist.device)  # Create tensor with the same device as dist

        return rank1, mAP, rank1_rerank, mAP_rerank, rank1_tlift, mAP_tlift, dist.cpu().numpy(), dist_rerank.cpu().numpy(), dist_tlift.cpu().numpy(), pre_tlift_dict
