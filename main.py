...
from reid.models.classifier import Classifier, NormalizedClassifier
...
def get_data(dataname, data_dir, model, matcher, save_path, args):
    ...
    if dataset.num_train_ids is not None:
...
def get_test_prcc_data(dataname, data_dir, height, width, workers=8, test_batch=64):
    root = osp.join(data_dir, dataname)
    ...
    query_sc_loader = DataLoader(
        Preprocessor(dataset.query_same,
                     root=osp.join(dataset.images_dir, dataset.query_path), transform=test_transformer),
        batch_size=test_batch, num_workers=workers,
        shuffle=False, pin_memory=True)

    query_cc_loader = DataLoader(
        Preprocessor(dataset.query_diff,
                     root=osp.join(dataset.images_dir, dataset.query_path), transform=test_transformer),
        batch_size=test_batch, num_workers=workers,
        shuffle=False, pin_memory=True)
    ...
    return dataset, query_sc_loader, query_cc_loader, gallery_loader

def get_test_cc_data(dataname, data_dir, height, width, workers=8, test_batch=64):
    root = osp.join(data_dir, dataname)
...
def show_info(args, info_type, testset, test_name, test_time, test_rank1, test_mAP, test_rank1_rerank, test_mAP_rerank,
                test_rank1_tlift, test_mAP_tlift, test_dist, test_dist_rerank,
                test_dist_tlift, pre_tlift_dict, json_file):
...
def main(args):
    ...
    # classifier
    clothing_classifier = Classifier(args.neck * hei * wid, num_classes).cuda()
    id_classifier = NormalizedClassifier(args.neck * hei * wid, num_classes).cuda()

    # Criterion
    criterion = TripletLoss(matcher, margin=args.margin, clothing_classifier=clothing_classifier, id_classifier=id_classifier, num_classes=num_classes).cuda()
    ...
    for test_name in test_names:
        ...
        #         # or (args.evaluate and args.basemodel == "prcc")):
        ...
        #       # or (args.evaluate and args.basemodel in ['ltcc', 'deepchange', 'last', 'vcclothes'])):
        ...
        if (args.testset == 'prcc'):
            print(args.testset, "<<<")
            testset, test_query_sc_loader, test_query_cc_loader, test_gallery_loader = get_test_prcc_data(test_name, args.data_dir, args.height, args.width, args.workers, args.test_fea_batch)
            # sc
            test_rank1, test_mAP, test_rank1_rerank, test_mAP_rerank, test_rank1_tlift, test_mAP_tlift, test_dist, test_dist_rerank, test_dist_tlift, pre_tlift_dict = evaluator.evaluate(testset, test_query_sc_loader, test_gallery_loader)
            info_type = ">>> SC:"
            test_time = 0
            show_info(args, info_type, testset, test_name, test_time, test_rank1, test_mAP, test_rank1_rerank, test_mAP_rerank,
                    test_rank1_tlift, test_mAP_tlift, test_dist, test_dist_rerank,
                    test_dist_tlift, pre_tlift_dict, json_file)
            # cc
            test_rank1, test_mAP, test_rank1_rerank, test_mAP_rerank, test_rank1_tlift, test_mAP_tlift, test_dist, test_dist_rerank, test_dist_tlift, pre_tlift_dict = evaluator.evaluate(testset, test_query_cc_loader, test_gallery_loader)
            info_type = ">>> CC:"
            show_info(args, info_type, testset, test_name, test_time, test_rank1, test_mAP, test_rank1_rerank, test_mAP_rerank,
                    test_rank1_tlift, test_mAP_tlift, test_dist, test_dist_rerank,
                    test_dist_tlift, pre_tlift_dict, json_file)
        elif (args.testset in ['ltcc', 'deepchange', 'last', 'vcclothes']):
        ...
        test_rank1, test_mAP, test_rank1_rerank, test_mAP_rerank, test_rank1_tlift, test_mAP_tlift, test_dist, test_dist_rerank, test_dist_tlift, pre_tlift_dict = evaluator.evaluate(testset, test_query_loader, test_gallery_loader)
